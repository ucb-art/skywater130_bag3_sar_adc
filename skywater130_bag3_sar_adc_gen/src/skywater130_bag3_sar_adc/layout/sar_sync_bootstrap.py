...
class SARSliceBootstrap(TemplateBase):
    ...
    def get_schematic_class(self) -> Optional[Type[Module]]:
        ...
        return ModuleDB.get_schematic_class('skywater130_bag3_sar_adc', 'sar_slice_wsamp' if self.params['sampler_params'] else
                                            'sar_slice_bot')
    ...
    def draw_layout(self) -> None:
        ...
        logic_x = -(-(w_tot - w_logic)//(2*w_conn_blk))*w_conn_blk
        comp_x = -(-(w_tot - w_comp) // (2*w_conn_blk))*w_conn_blk
        dac_x = -(-(w_tot - 2 * w_dac) // (2*w_conn_blk))*w_conn_blk
        ...
        if sampler_params:
            ...
            sampler = self.add_instance(sampler_master, inst_name='XSAM',
                              xform=Transform(-(-(w_tot-w_sam)//(2*w_conn_blk))*w_conn_blk, sam_y)) #mode=Orientation.MY))
            ...
            self.reexport(sampler.get_port('vcm'), net_name='vref<1>', connect=True) 
        ...
        dac_n_vm = self.connect_to_tracks(dac_p_xm, TrackID(vm_layer, dac_top_p[0].track_id.base_index, tr_w_sig_xm))
        dac_p_vm = self.connect_to_tracks(dac_n_xm, TrackID(vm_layer, dac_top_n[0].track_id.base_index, tr_w_sig_xm))
